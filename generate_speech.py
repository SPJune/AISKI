...
def read_voice_map(voice_info_path: Path) -> Dict[str, str]:
    ...
    Expected headers include: name, voice_id
    """
    ...
    with voice_info_path.open(newline="", encoding="utf-8") as f:
        ...
        if not required.issubset(set(reader.fieldnames or [])):
            raise ValueError(
                f"Missing required headers in voice_info CSV. Required: {required}, "
...
def fetch_audio_bytes_from_tts(
    ...
    """
    Call Typecast TTS.
    - If response is audio bytes, return directly.
    - If response is JSON containing audio URL/path, download audio from it.
    - audio_tempo: speech speed (API range typically 0.5–2.0; default 1.0).
    ...
    headers = {
        ...
        "Content-Type": "application/json",
        # Some APIs support direct audio with Accept header.
    }
    '''
    payload = {
        "voice_id": voice_id,
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 2) JSON response containing URL/path.
            # Try common candidate keys.
            ...
            if status not in (429, 500, 502, 503, 504) or attempt == max_retries:
                # Keep detailed error text when possible.
                try:
...
def process_csv(
    ...
    out_dir: Path = Path("outputs/stimulus"),
) -> None:
    ...
    with pilot_csv_path.open(newline="", encoding="utf-8-sig") as f:
        ...
        if not required_cols.issubset(set(reader.fieldnames or [])):
            raise ValueError(
                f"Missing required columns in input CSV. Required: {required_cols}, "
                f"actual: {reader.fieldnames}"
            )
        ...
        for row in reader:
            ...
            if not name or not text or not stim:
                print(f"[SKIP] Missing required values: stimulus_num={stim!r}, name={name!r}, utterance(len)={len(text)}")
                skipped += 1
                continue
            ...
            fname = f"stimulus{stim}_{safe_filename(name)}.wav"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate speech WAV files from aligned CSV using Typecast"
    )
    parser.add_argument("pilot_csv", help="Path to aligned input CSV")
    parser.add_argument(
        "--voice-info",
    ...
    process_csv(pilot_csv_path, voice_info_path)
